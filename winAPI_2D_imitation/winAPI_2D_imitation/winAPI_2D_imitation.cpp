// winAPI_2D_imitation.cpp : 애플리케이션에 대한 진입점을 정의합니다.
//
                     ...
                     _In_ int       nCmdShow)
{
    ...
    // GetMessage  : 메시지 큐에 메세지가 없다면 대기, 메시지가 들어온다면 true 반환
    // PeekMessage : 메시지 큐에 메세지가 없다면 false 반환, 메시지가 있다면 true 반환
...
ATOM MyRegisterClass(HINSTANCE hInstance)
{
    ...
    wcex.cbClsExtra     = 0;                         // 윈도우 클래스에서 사용하고자하는 여분의 메모리양을 바이트 단위로 지정 
    wcex.cbWndExtra     = 0;                         // cbClsExtra와 유사하나 개별 윈도우에서 사용고자 하는 여분의 메모리양을 지정   
    ...
    wcex.lpszMenuName   = nullptr;                                                     // 윈도우에서 사용할 메뉴 지정, nullptr롤 없앰 
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_COMMAND:
        ...
        break;
    case WM_PAINT:  // 윈도우의 작업영역이 다시 그려져야 할 때 실행됨. (무효화 영역이 발생 했을 때)
        {
            PAINTSTRUCT ps;
